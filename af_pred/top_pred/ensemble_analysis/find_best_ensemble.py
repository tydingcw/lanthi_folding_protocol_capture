...
def parse_file(filename, scorefile, energy=0.2, isomer_list=None): #, energy_list = [], pred_shift=None, obs_shift=None):
	#def parse_file(filename, scorefile, energy=0.2, pred_shift=None, obs_shift=None):
...
#	if (energy_list == [] or pred_shift==None or obs_shift==None) :
	if isomer_list != None:
		for desc in df['description']:
			temp_desc = desc.split('/')[-1]
			if temp_desc in isomer_list:
				structs.append(temp_desc)
				base = desc.split('/')[0]
	else:
...
#			shifr_score = score_sing_shift(pred_shift, obs_shift[desc])
#			if shift_score < 1000:
#				structs.append(desc)
#		print(f'passing shifr filter: {len(structs)}')
...
def parse_obs_shift(pred_shift):
    csv_str = ''
    index_count = 0 
    resi_col = 0 
    shift_col = 0 
    ...
    shift_dict = {}
    reading = False
    with open(pred_shift, 'r') as in_file:
        for raw_line in in_file:
            ...
            if 'Atom_chem_shift' in line:
                ...
                reading = True
                if 'Seq_ID' in line:
                   resi_col = index_count
                elif 'Comp_ID' in line:
                    comp_col =  index_count
                elif 'Atom_ID' in line:
                    atom_col =  index_count
                elif 'Val' == line.split('.')[-1]:
                    shift_col = index_count
                index_count += 1
            elif reading and 'stop_' in line:
                break
            elif reading and len(split) > shift_col and split[atom_col] not in ['DBU']:
                #grab those chemical shifts
                shift_dict[f'{split[resi_col]}_{split[atom_col]}'] = float(split[shift_col])
...
def parse_pred_shift(pred_shift):
    with open(pred_shift) as in_file:
        first_line = in_file.readline()
        cols=first_line.split(',')
    #print('***********************')
    #print(cols)
    #df = pd.read_csv(pred_shift, delim_whitespace=False)[cols]
    df = pd.read_csv(pred_shift, delim_whitespace=False)
    #print(df)
    ...
    for desc in df['description']:
        ...
        df_temp = df.loc[df['description'] == desc ]
        shift_dict[desc] = {}
        for col in [x for x in cols if 'description' not in x]:
            measure = float(df_temp[col])
            shift_dict[desc][col] = measure
...
# monte carlo moves
def mc(ncycles,dataset,reference,mode,temp,negative=None, neg_cutoff=4.0, pred_shift=None, obs_shift=None, shift_weight=1.0):
	...
	for i in range(ncycles):
		...
		#with open('best_score.txt', 'a') as shift_file: #TODO remove
		#	shift_file.write(f'{best_score}\n')
		# 1. make some move
		...
		elif pred_shift != None and obs_shift != None:
			current_score = score_shift(current_ensemble,reference, pred_shift, obs_shift, shift_weight=shift_weight)
		else:
...
def score_shift(current_set, reference, pred_shift, obs_shift, shift_weight=1.0):  # current_set is a list of maps (filename, map)
	score_sum = 0
	neg_map = {}
	score_sum = score(current_set, reference)

	#Calculate the chemical shift score here
	#Take the average shift of the ensemble for the specific atom
	#square the deviation from the measured shift (root mean of this potentially)
	#May need to reweight the shifts to account for smaller deviations compared to NOE
	shift_score = 0
	first_struct = list(pred_shift.keys())[0]
	deviations = []
	for key in [x for x in list(obs_shift.keys()) if x in pred_shift[first_struct].keys()]: #all shifts except first and last
		diff_vals = []
		for filename, data in current_set:
			#get the shift value
			shift_val = pred_shift[filename][key]
			deviation = shift_val - obs_shift[key] #TODO reweight the deviations
			#if 'H' in key:
			#	deviation = deviation * 0 #was 10
			diff_vals.append(deviation)
			#deviation = 0
			#TODO change the scoring to be ensemble based - RMSD or average and square diff to experiment
			#if shift_val >= pred_shift[key][0] and shift_val <= pred_shift[key][1]:
			#	pass #deviation is 0
			#else:
			#	#need to see how far outside of the range we are
			#	#https://gamedev.stackexchange.com/questions/4467/comparing-angles-and-working-out-the-difference
			#	dev1 = 180 - abs(abs(shift_val - pred_shift[key][0]) - 180); 
			#	dev2 = 180 - abs(abs(shift_val - pred_shift[key][1]) - 180); 
		...
		#diff = min(neg_map[key] - cutoff, 0) ** 2
		#sum += diff
		#score_sum += np.mean(diff_vals)**2
		shift_score += np.mean(diff_vals)**2
	#with open('shift_file.txt', 'a') as shift_file: #TODO remove
	#	shift_file.write(f'{shift_score}\n')
	#sum + shift_score
	return score_sum + (shift_score * shift_weight)
...
parser.add_argument("--pred_shift",dest="pred_shift",help="file with predicted  NMR CS, intended for sparta+ predictions",default="")
parser.add_argument("--obs_shift",dest="obs_shift",help="file with observed NMR CS for the top quantile of structures",default="")
parser.add_argument("--shift_weight",dest="shift_weight",help="weight to give to chemical shifts", type=float, default=0.8)
parser.add_argument('-f', "--isomer_csv",help="file with a csv for all structures of a desired atropisomer",default="")
...
if args.isomer_csv != '':
    df = pd.read_csv(args.isomer_csv, delim_whitespace=False)
    isomer_list = list(df['description'])
    #print(isomer_list)
    dataset, base = parse_file(args.input, args.scorefile, energy=args.energy, isomer_list=isomer_list)
else:
...
if (args.pred_shift != '' and args.obs_shift == '') or (args.pred_shift == '' and args.obs_shift != ''):
    print('both pred_shift and obs_shift should be defined or undefined')
    exit(1)

pred_shift_vals = None
if args.pred_shift != '':
    pred_shift_vals = parse_pred_shift(args.pred_shift)

obs_shift_vals = None
if args.obs_shift != '':
    #obs_shift_vals = parse_obs_shift(args.obs_shift, dataset.keys())
    obs_shift_vals = parse_obs_shift(args.obs_shift)
#print "...done."
...
elif args.pred_shift != '' and args.obs_shift != '':
	#TODO refilter dataset based on dihedrals
	#dataset, temp = parse_file(args.input, args.scorefile, energy=args.energy, energy_list=dataset.keys(), pred_shift=pred_shift_vals, obs_shift=obs_shift_vals)
	mc_result = (mc(ncycles,dataset,reference,args.mode,args.temp, pred_shift=pred_shift_vals, obs_shift=obs_shift_vals, shift_weight=args.shift_weight))
else:
