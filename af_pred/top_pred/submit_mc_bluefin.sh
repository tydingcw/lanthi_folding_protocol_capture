...
BASE=$1
for item in `seq 0 4`; do 
count=`cat ${BASE}_top.txt | grep ${BASE}/sample_${BASE}_${item}_ | wc -l`; 

if [ $count -ge 1 ]; then 
#echo $count
cat ${BASE}_top.txt | grep ${BASE}/sample_${BASE}_${item}_ > temp_pdb_names/${BASE}_pdbs_${item}.txt 
